...
from RET_CLIP_PLUS.clip.model import CLIP, resize_pos_embed
...
def eval_multiLabelCls_ViT(model, device):
    ...
    for epoch in range(100):
        ...
        for step in tqdm(range(len(data_iter))):
            ...
            if LINEAR_PROBING:
                with torch.no_grad():
                    image_features = model.get_img_feats(imgs)
            else:
                image_features = model.get_img_feats(imgs)
...
def main():
    ...
    for i in range(5):
        clip_resume = '/home/ubuntu/nfs/8T2/dujw/experiments/Both_original_and_MAE_as_input/checkpoints/best/epoch2.pt'
        # clip_resume = '/home/ubuntu/nfs/8T2/dujw/ret-clip.pt'

        vision_model_config_file = "../RET_CLIP_PLUS/clip/model_configs/ViT-B-16.json"
        print('Loading vision model config from', vision_model_config_file)
        assert os.path.exists(vision_model_config_file), "The vision_model_config_file does not exist!"

        text_model_config_file = "../RET_CLIP_PLUS/clip/model_configs/RoBERTa-wwm-ext-base-chinese.json"
        ...
        log_path = os.path.join(log_path, "RET-CLIP-PLUS_ft_{}.log".format(time_suffix))
