...
def eval_multiLabelCls_ViT(model, device):
    ...
    # fine-tuning
        ...
        optimizer = torch.optim.AdamW(
            [
                {"params": model.parameters(), "lr": 2e-6, "weight_decay": 0.001, "betas": (0.9, 0.98)},
...
def main():
    ...
    for i in range(5):
        pretrained = "/home/ubuntu/nfs/8T1/dujw/clip/datapath_clip/pretrained_weights/New Folder"
        model = AutoModel.from_pretrained(pretrained, device_map=device)

        time_suffix = strftime("%Y-%m-%d-%H-%M-%S", gmtime())
        log_path = '/home/ubuntu/nfs/8T1/dujw/clip/downstream_train_valid_test/FIVES'
        log_path = os.path.join(log_path, "dinov3_test_{}.log".format(time_suffix))
