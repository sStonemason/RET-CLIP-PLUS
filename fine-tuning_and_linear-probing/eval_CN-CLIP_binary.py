...
def eval_multiLabelCls_ViT(model, device):
    ...
    # fine-tuning
        ...
        optimizer = torch.optim.AdamW(
            [
                {"params": gain_or_bias_params, "lr": 5e-6, "weight_decay": 0., "betas": (0.9, 0.98)},
                {"params": rest_params, "lr": 5e-6, "weight_decay": 0.001, "betas": (0.9, 0.98)},
                {"params": classifier.parameters(), "lr": 1e-3, "weight_decay": 0.01, "betas": (0.9, 0.99)},
            ],
            eps=1e-6,
        )
...
def main():
    ...
    for i in range(5):
        ...
        log_path = os.path.join(log_path, "cn_clip_ft_{}.log".format(time_suffix))
