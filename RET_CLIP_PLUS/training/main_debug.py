...
# # 2023.10.19 for debugging
# import torch.distributed.launch
import torch.multiprocessing as mp

from RET_CLIP_v2.clip import load
from RET_CLIP_v2.clip.model import convert_weights, convert_state_dict, resize_pos_embed, CLIP
from RET_CLIP_v2.training.train import train, evaluate
from RET_CLIP_v2.training.data import get_data
from RET_CLIP_v2.training.params_debug import parse_args
from RET_CLIP_v2.training.logger import setup_primary_logging, setup_worker_logging
from RET_CLIP_v2.training.scheduler import cosine_lr
...
def debug(rank, world_size=1):
    ...
    #     '--master_port', '8529',
    #     '--use-env', '/home/ubuntu/nfs/2T2/dujw/clip/Chinese-CLIP/cn_clip/training/main.py',
    #     '--train-data', '/home/ubuntu/nfs/2T2/dujw/clip/DATAPATH_clip/datasets/tongren_v3_512/lmdb/train',
    #     '--val_data', '/home/ubuntu/nfs/2T2/dujw/clip/DATAPATH_clip/datasets/tongren_v3_512/lmdb/valid',
    #     '--num-workers', '16',
    #     '--resume', '/home/ubuntu/nfs/2T2/dujw/clip/DATAPATH_clip/pretrained_weights/clip_cn_vit-b-16.pt',
    #     '--logs', '/home/ubuntu/nfs/2T2/dujw/clip/DATAPATH_clip/experiments/',
    #     '--name', 'tongren_v3_512_trainval_uncrossed_vit-b-16_RoBERTa-wwm-ext-base_bs256_augment_changed',
    #     '--save-step-frequency', '9999999999',
    ...
    # ]
    # torch.distributed.launch.main(launch_args)
    # # 2023.10.19 end
    args = parse_args()
    ...
    if args.resume is None:
        ...
        if args.resume_visual is not None:
            if os.path.isfile(args.resume_visual):
                logging.info(f"=> begin to load vision encoder '{args.resume_visual}'")
                vision_encoder = torch.load(args.resume_visual, map_location="cpu")
                sd_vision = {k: v for k, v in vision_encoder["state_dict"].items() if "bert" not in k}
                resize_pos_embed(sd_vision, model, prefix="module.")

                # Adapt flash attention
                if args.use_flash_attention:
                    sd_vision = convert_state_dict(sd_vision)

                # Load the state dict
                # 2023.10.12 support for different input resolution
                if int(model_info['image_resolution']) != 224:
                    del sd_vision["module.visual.attnpool.positional_embedding"]
                ...
                if not args.reset_data_offset:
                    start_epoch = vision_encoder["epoch"]
                    steps = vision_encoder["step"]
                    data = get_data(args,
                ...
                if not args.reset_optimizer and optimizer is not None:
                    optimizer.load_state_dict(vision_encoder["optimizer"])
                    logging.info("=> optimizer state is restored from the checkpoint")

                logging.info(
                    f"=> loaded vision encoder '{args.resume_visual}' (epoch {vision_encoder['epoch']} @ {steps} steps)"
                )

        if args.resume_text is not None:
            if os.path.isfile(args.resume_text):
                logging.info(f"=> begin to load text encoder '{args.resume_text}'")
                text_encoder = torch.load(args.resume_text, map_location="cpu")
                sd_text = {k: v for k, v in text_encoder["state_dict"].items() if
                           "bert" in k and "bert.pooler" not in k}
                resize_pos_embed(sd_text, model, prefix="module.")
                # Adapt flash attention
                if args.use_flash_attention:
                    sd_text = convert_state_dict(sd_text)

                # Load the state dict
                # 2023.10.12 support for different input resolution
                model.load_state_dict(sd_text, strict=False)

                # Restore the epoch and steps info, reload the dataset and dataloader for the resume epoch
                if not args.reset_data_offset:
                    start_epoch = text_encoder["epoch"]
                    steps = text_encoder["step"]
                    data = get_data(args,
                                    epoch_id=start_epoch,
                                    max_txt_length=args.context_length)
                # Restore the optim state
                if not args.reset_optimizer and optimizer is not None:
                    optimizer.load_state_dict(text_encoder["optimizer"])
                    logging.info("=> optimizer state is restored from the checkpoint")

                logging.info(
                    f"=> loaded text encoder '{args.resume_text}' (epoch {text_encoder['epoch']} @ {steps} steps)"
                )
    ...
    if args.resume is not None:
        if os.path.isfile(args.resume):
            ...
            # 2024.1.5 triple_bert
            # sd_bert_left = {k.replace('bert', 'bert_left'): v for k, v in checkpoint["state_dict"].items() if
            #            "bert" in k and "bert.pooler" not in k}
            # sd_bert_right = {k.replace('bert', 'bert_right'): v for k, v in checkpoint["state_dict"].items() if
            #            "bert" in k and "bert.pooler" not in k}
            # sd.update(sd_bert_left)
            # sd.update(sd_bert_right)
