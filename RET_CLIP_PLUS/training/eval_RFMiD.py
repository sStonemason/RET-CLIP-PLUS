...
class classifier_cls(nn.Module):
    def __init__(self, num_classes):
        ...
        self.fc_norm = norm_layer(768)

    def forward(self, x):
        patch_feat = x[:, 1:, :].mean(dim=1)  # global pool without cls token
        patch_feat = self.fc_norm(patch_feat)
        feat = torch.cat((x[:, 0, :], patch_feat), dim=-1)
...
def eval_multiLabelCls_ViT(model, epoch_main, cuda_rank):
    ...
    TRAIN = True
    # DATA_DIR = '/data/disk8T1/dujw/clip/datapath_clip/datasets/RFMID'
    ...
    # classifier = classifier_cls(num_classes).to(device)
    ...
    if TRAIN:
        ...
        for epoch in range(75):
            ...
            for step in tqdm(range(len(data_iter))):
                ...
                image_features = model.module.get_img_feats(imgs)
