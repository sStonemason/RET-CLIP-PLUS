...
class LMDBDataset(Dataset):
    def __init__(self, lmdb_path, split="val", max_txt_length=64, use_augment=False, resolution=224):
        ...
        # self.my_trans = self.build_my_transform(resolution)
    ...
    # def build_my_transform(self, resolution):
    #     transform = transforms.Compose([
    #         transforms.Resize((resolution, resolution)),
    #         transforms.RandomRotation(degrees=30, interpolation=transforms.InterpolationMode.BICUBIC),
    #         transforms.RandomResizedCrop((resolution, resolution), scale=(0.8, 1.0), ratio=(0.7, 1.3)),
    #         transforms.RandomHorizontalFlip(p=0.5),
    #         # transforms.RandomVerticalFlip(p=0.5),
    ...
    def __getitem__(self, index):  # 通过index来getitem
        sample_index = index % self.number_samples

        pair = pickle.loads(self.txn_pairs.get("{}".format(sample_index).encode('utf-8')).tobytes())
        # 2023.11.7
        patient_id, text_id, raw_text = pair

        # 获取复合值
        imgs = self.txn_imgs.get("{}".format(patient_id).encode('utf-8')).tobytes()
        images_b64 = pickle.loads(imgs)
        # 按需获取两张图片
        img_b64_l = images_b64[0]
        img_b64_r = images_b64[1]
        img_l_o = Image.open(BytesIO(base64.urlsafe_b64decode(img_b64_l)))
        img_r_o = Image.open(BytesIO(base64.urlsafe_b64decode(img_b64_r)))
        img_l = self.transform(img_l_o)
        img_r = self.transform(img_r_o)
        img_l_p = self.transform(img_l_o)
        img_r_p = self.transform(img_r_o)

        # image_b64 = self.txn_imgs.get("{}".format(patient_id).encode('utf-8')).tobytes()
        ...
        return img_l, img_r, img_l_p, img_r_p, text, eos_index
...
#             transforms.Resize((resolution, resolution)),
#             transforms.RandomRotation(degrees=30, interpolation=transforms.InterpolationMode.BICUBIC),
#             transforms.RandomResizedCrop((resolution, resolution), scale=(0.7, 1.3), ratio=(0.7, 1.3)),
#             transforms.RandomHorizontalFlip(p=0.5),
#             # transforms.RandomVerticalFlip(p=0.5),
#             transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0., hue=0.),
#             transforms.ToTensor(),
...
#         img_r = self.transform(img_r_o)
#
#         img_p_l = self.my_trans(img_l_o)
#         img_p_r = self.my_trans(img_r_o)
#
#         # image_b64 = self.txn_imgs.get("{}".format(patient_id).encode('utf-8')).tobytes()
#         # image_b64 = image_b64.decode(encoding="utf8", errors="ignore")
#         # image = Image.open(BytesIO(base64.urlsafe_b64decode(image_b64)))  # already resized
#         # image = self.transform(image)
#         # def find_commas(s):
#         #     return [index + 1 for index, char in enumerate(s) if char == '，' or char == ',']
#
#         text = tokenize([_preprocess_text(raw_text)], context_length=self.max_txt_length)[0]
#
#         # commas = find_commas(raw_text)
#         eos_index = text.numpy().tolist().index(_tokenizer.vocab['[SEP]'])
#         # commas.append(eos_index)
#         # text_dict = {"commas": commas,
#         #              "text": text}
#         return img_l, img_r, text, eos_index, img_p_l, img_p_r
#
#
# def pad_dataset(dataset, global_batch_size):
#     # edit dataset.__len__() of the dataset
