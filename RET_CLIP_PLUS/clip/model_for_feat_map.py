...
class Transformer(nn.Module):
    ...
    def forward_intermediate_outputs(self, x: torch.Tensor):
        intermediate_outputs = []  # 用于保存中间输出
        for i, block in enumerate(self.resblocks):
            if self.grad_checkpointing and not torch.jit.is_scripting():
                # 使用grad checkpointing
            ...
            if (i + 1) % 3 == 0:
                ...
                # return x
                processed_feature = self.process_feature(x)
                intermediate_outputs.append(processed_feature)
...
class VisualTransformer(nn.Module):
    ...
    def add_jitter(self, feature_tokens, scale, prob):
        if random.uniform(0, 1) <= prob:
            ...
            jitter = torch.randn((batch_size, num_tokens, dim_channel)).cuda()
    ...
    def forward(self, x: torch.Tensor, mask_ratio: float = 0.0, as_dae_encoder=False):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]

        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        # if as_dae_encoder:
        #     x = self.add_jitter(x, scale=10, prob=1.0)
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        x = x + self.positional_embedding.to(x.dtype)
    ...
    def foward_as_dae_encoder(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        ...
        print('as dae encoder...')
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer.forward_intermediate_outputs(x)
        # x = torch.cat(x, dim=-1)
        # print('融合特征：')
        # print(x.shape)
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class MAE_decoder(nn.Module):
    def __init__(self, embed_dim=768, decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16, patch_size=16,
        ...
        # self.decoder_embed = nn.Sequential(nn.Linear(embed_dim * 4, embed_dim), nn.ReLU(),
        #                                    nn.Linear(embed_dim, decoder_embed_dim), nn.ReLU(),
...
class DAE_decoder(nn.Module):
    def __init__(self, embed_dim=768, decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16, patch_size=16,
        ...
        self.decoder_embed = nn.Sequential(nn.Linear(embed_dim, decoder_embed_dim), nn.ReLU(),
                                           nn.Linear(decoder_embed_dim, decoder_embed_dim))
...
class CLIP(nn.Module):
    def  __init__(self,
        ...
        #
        # self.global_feature_mapping_rec = nn.Linear(2 * embed_dim, embed_dim, bias=False)
        # self.left_feature_mapping_rec = nn.Linear(embed_dim, embed_dim, bias=False)
        # self.right_feature_mapping_rec = nn.Linear(embed_dim, embed_dim, bias=False)

        # self.predictor = nn.Sequential(nn.Linear(embed_dim, embed_dim, bias=False),
    ...
    def set_grad_checkpointing(self, enable=True):
        ...
        # self.DAE_decoder.grad_checkpointing = enable
    ...
    def get_attn_map(self, x, imgs_path):
        attn = self.visual.get_last_attn(x.type(self.dtype))
        import os
        ...
        w_featmap = 14
        h_featmap = 14
        output_dir = '/home/ubuntu/nfs/8T2/dujw/experiments/Both_original_and_MAE_as_input/attn_map'
        os.makedirs(output_dir, exist_ok=True)
        attn_list = []

        bs = x.shape[0]
        for i in range(bs):
            img_name = imgs_path.split()
            torchvision.utils.save_image(torchvision.utils.make_grid(x[i], normalize=True, scale_each=True),
                                         os.path.join(output_dir, "img_" + str(i) + ".png"))
            ...
            fname = os.path.join(output_dir, "img_" + str(i) + "_attn-map" + "_mean" + ".png")
            plt.imsave(fname=fname, arr=attn_mean, format='png')

    def encode_image_mae_input(self, pred, mask_ratio=0):
        left_feature = self.visual(pred[:128].type(self.dtype), mask_ratio)
        right_feature = self.visual(pred[128:].type(self.dtype), mask_ratio)
        vision_feature = torch.cat(
            (left_feature, right_feature), dim=1)

        return self.left_feature_mapping(
            left_feature), self.right_feature_mapping(right_feature), self.global_feature_mapping(
            vision_feature)
        # return self.global_feature_mapping(vision_feature)

    def encode_image_dae_input(self, pred):
        print('---->dae_input: ')
        left_feature = self.visual(pred[:128].type(self.dtype), mae_input=True)
        right_feature = self.visual(pred[128:].type(self.dtype), mae_input=True)
        vision_feature = torch.cat(
            (left_feature, right_feature), dim=1)
    ...
    def DAE_loss(self, imgs, pred, norm_pix_loss=False):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        """
    ...
    def DAE_encoder(self, x):
        latent = self.visual.foward_as_dae_encoder(x)
        return latent

    def forward(self, img_l, img_r, text=None, epoch=0, mask_ratio=0):
        ...
        left_features, right_features, image_features = self.encode_image(img_l, img_r, img_l_p, img_r_p, mask_ratio)
        ...
        # latent = self.DAE_encoder(imgs)
        # pred = self.DAE_decoder(latent)
        # DAE_loss = self.DAE_loss(imgs, pred, norm_pix_loss=True)
        ...
        # image_features = self.encode_image_mae_input(pred)

        # image_features = self.encode_image_dae_input(pred)
        ...
        return image_features, text_features, text_features_left, text_features_right, left_features, right_features, left_features_rec, right_features_rec, image_features_rec, self.logit_scale.exp(), self.logit_scale_left.exp(), self.logit_scale_right.exp(), MAE_loss
